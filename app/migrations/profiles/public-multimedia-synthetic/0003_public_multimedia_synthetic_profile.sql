CREATE TABLE fixture_profile_ledger (
  profile_id TEXT PRIMARY KEY CHECK (profile_id = 'public-multimedia-synthetic'),
  sqlite_path TEXT NOT NULL UNIQUE,
  contract_version TEXT NOT NULL CHECK (contract_version = 'public-read-v0.2'),
  ...
  manifest_root_sha256 TEXT NOT NULL CHECK (length(manifest_root_sha256) = 64),
  profile_ledger_root_sha256 TEXT NOT NULL CHECK (length(profile_ledger_root_sha256) = 64),
  generator_root_sha256 TEXT NOT NULL CHECK (length(generator_root_sha256) = 64),
  validator_root_sha256 TEXT NOT NULL CHECK (length(validator_root_sha256) = 64),
  ...
  real_media INTEGER NOT NULL CHECK (real_media = 0),
...
CREATE TABLE public_media_candidate (
  ...
  content_id TEXT NOT NULL REFERENCES public_content(content_id) ON UPDATE RESTRICT ON DELETE RESTRICT,
  media_hash TEXT NOT NULL UNIQUE CHECK (length(media_hash) = 64),
  candidate_status TEXT NOT NULL CHECK (candidate_status = 'selected'),
  payload_json TEXT NOT NULL,
  UNIQUE (content_id, media_candidate_id)
);

CREATE TRIGGER public_media_candidate_max_four_before_insert
BEFORE INSERT ON public_media_candidate
WHEN (SELECT COUNT(*) FROM public_media_candidate WHERE content_id = NEW.content_id) >= 4
BEGIN
  SELECT RAISE(ABORT, 'PUBLIC_MEDIA_LIMIT_EXCEEDED');
END;

CREATE TRIGGER public_media_candidate_max_four_before_content_update
BEFORE UPDATE OF content_id ON public_media_candidate
WHEN NEW.content_id <> OLD.content_id
 AND (SELECT COUNT(*) FROM public_media_candidate WHERE content_id = NEW.content_id) >= 4
BEGIN
  SELECT RAISE(ABORT, 'PUBLIC_MEDIA_LIMIT_EXCEEDED');
END;
...
CREATE TABLE public_release_bundle (
  ...
  media_presentations_json TEXT NOT NULL,
  payload_json TEXT NOT NULL
);
