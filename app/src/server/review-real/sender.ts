...
export interface ProjectionSenderTransport {
  post(packageValue: SignedProjectionPackage): Promise<ProjectionTransportResult>;
...
function requestJson(input: Readonly<{
  method: "GET" | "POST";
  path: string;
  body?: unknown;
  ...
  return new Promise((resolveResult) => {
    const body = input.body === undefined ? null : Buffer.from(JSON.stringify(input.body), "utf8");
    const request = httpRequest({
      ...
      response.on("data", (chunk: Buffer | string) => {
        ...
        if (size > MAX_RECEIPT_BYTES) request.destroy(new Error("PROJECTION_RESPONSE_TOO_LARGE"));
      ...
      response.on("end", () => {
        ...
        if (size > 0) {
          ...
          catch { resolveResult({ kind: "response", status: Number(response.statusCode ?? 500), body: null }); return; }
        }
        resolveResult({ kind: "response", status: Number(response.statusCode ?? 500), body: value });
      });
    });
    request.setTimeout(input.timeoutMs, () => request.destroy(new Error("PROJECTION_REQUEST_TIMEOUT")));
...
export class ProjectionHttpTransport implements ProjectionSenderTransport {
  ...
  constructor(input: Readonly<{
    ...
    if (input.endpoint !== INTERNAL_ENDPOINT || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,255}$/.test(input.serviceIdentity)) {
...
export type ProjectionSenderTickResult = Readonly<{
  outcome: "idle" | "busy" | "succeeded" | "reconcile_wait" | "retryable_failed" | "terminal_failed";
  deliveryId: string | null;
}>;

function exactReceipt(work: ProjectionDeliveryWork, body: unknown): unknown | null {
  const parsed = ProjectionReceiptSchema.safeParse(body);
  if (
    !parsed.success ||
    parsed.data.deliveryId !== work.deliveryId ||
    parsed.data.snapshotGeneration !== work.envelope.snapshot.snapshotGeneration ||
    parsed.data.snapshotManifestHash !== work.envelope.snapshot.snapshotManifestHash
  ) {
    return null;
  }
  return parsed.data;
...
export class ProjectionSender {
  ...
  private async deliver(work: ProjectionDeliveryWork): Promise<ProjectionSenderTickResult> {
    ...
    try {
      result = await this.transport.post(packageValue);
    } catch {
      result = { kind: "unknown" };
    }
    if (result.kind === "unknown") {
      this.repository.markDeliveryReconcileWait(work, "DELIVERY_RESPONSE_UNKNOWN", this.actorRef);
      return { outcome: "reconcile_wait", deliveryId: work.deliveryId };
    }
    if (result.status >= 200 && result.status <= 299) {
      const receipt = exactReceipt(work, result.body);
      if (receipt === null) {
        this.repository.markDeliveryTerminal(work, "DELIVERY_RECEIPT_MISMATCH", this.actorRef);
    ...
    if (result.status >= 500 && result.status <= 599) {
      const outcome = this.repository.markDeliveryRetryable(work, "DELIVERY_HTTP_5XX", this.actorRef);
      return { outcome, deliveryId: work.deliveryId };
    }
    this.repository.markDeliveryTerminal(
      work,
      result.status === 409 ? "DELIVERY_SEMANTIC_CONFLICT" : "DELIVERY_REQUEST_REJECTED",
      this.actorRef
    );
    return { outcome: "terminal_failed", deliveryId: work.deliveryId };
  }

  private async reconcile(work: ProjectionDeliveryWork): Promise<ProjectionSenderTickResult> {
    let result: ProjectionTransportResult;
    try {
    ...
    if (result.kind === "unknown" || (result.status >= 500 && result.status <= 599)) {
      return { outcome: "reconcile_wait", deliveryId: work.deliveryId };
    }
    if (result.status === 404) {
      const outcome = this.repository.markDeliveryRetryable(work, "PROJECTION_RECEIPT_UNKNOWN", this.actorRef);
      return { outcome, deliveryId: work.deliveryId };
    }
    if (result.status >= 200 && result.status <= 299) {
      const receipt = exactReceipt(work, result.body);
    ...
    this.repository.markDeliveryTerminal(work, "DELIVERY_SEMANTIC_CONFLICT", this.actorRef);
    return { outcome: "terminal_failed", deliveryId: work.deliveryId };
  }
}
