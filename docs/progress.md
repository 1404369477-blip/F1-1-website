...
- ✅ **`C917F4` 恢复门合法控制工具链已在 disposable 闭合 (G1–G5)，未 commit、未写生产**:G1 `backup-recovery-point-register` 走 owner handoff→gateway 写入 `backup_recovery_point`，`valid_backup_recovery_point_v1` 出现 1 行且 `assertRecoveryPointBinding` 通过（先合法插入 outbox 头与 `projection_recovery_anchor` 首行；gateway 对齐 0007 允许该表 activate+INSERT）。G2 注册成功后原子写 `recovery-fence.json`：`writerReady=true` / `clockTrusted`（未来偏差≤120s）/ `lastSuccessfulRecoveryPointAt=SNAP ms`。G4 `quick-launch --until ready` 终态三元组 `phase=disabled / global_stop=stopped / recovery=ready`，既有到 live 测试不回归。G5 `--schema10-plist` 对参数化目录 `installed-not-loaded`，不打开 v1 库、不调 launchctl。G3 草案 `docs/decisions/system/2026-08-30-F1+1-0011-source-registry-rss-config-v2-draft.md`，未 apply。证据:`scratch/TASK-20260830-C917F4/report.md`。生产登记仍缺 `budget_account` 种子；fence 首次生产置 true 与 0011 apply 仍需用户单独确认。

- ⛔ **`BBFF2A` resume 后随即证实按现有代码无法闭合,已重新 blocked;新任务 `TASK-20260830-C917F4` 补工具链**:grok-4.6 只读勘探 + 主线程逐条源码抽查证实五缺口——G1 `backup_recovery_point` 全仓库零 TS 写入点(唯一合法路径是 gateway `backup_worker` handoff→write permit→INSERT,无 CLI 封装);G2 `recovery-fence.json` 只有 `prepareAdminDeployment` 写 false 的入口,无合法置 true 路径;G3 `source_registry_rss_config_v1` 四源占位 hash 受不可变 trigger 保护(`SOURCE_REGISTRY_CONFIG_IMMUTABLE`),替换需 additive 迁移(RETIER-001 已预告);G4 唯一控制 CLI `quick-launch-enter-live` 固定一推到 `phase=live`,会让 Admin sender 60s tick 消费那 1 条 `reconcile_wait`,与验收 reconcile=0 冲突,需要 `--until ready` 停点(schema 允许停在 ready+stopped+disabled);G5 `rss:install-macos` prepare 会以 v1 断言打开库,对 schema10 生产库必失败。缺口清单与状态机:`scratch/TASK-20260829-BBFF2A/control-path-gaps.md`。处置:BBFF2A 二次 blocked(新原因),`C917F4`(恢复门合法控制工具链补全 G1-G5)已 enqueue 并由开发部 claim,全程零生产写、disposable 验证,0011 迁移 apply 与 fence 首次生产置 true 仍留用户单独确认。勘探另证实:`rss-control.ts` 与恢复门无关;正式 schema verifier 是 `assertSourceRegistrySchema`(schema10 opener 内嵌,无独立 CLI)。

- ✅ **SNAP 已部署 M1 生产并产出第一个 verified 异机加密恢复点**：A 级动作,用户逐项拍板(异机目的地=iCloud Drive、密钥=用户手动托管密码管理器、首轮真实快照单独批准)。三份 SNAP 代码按 SHA-256 校验同步进生产树;LaunchAgent `com.f1plus1.backup-snapshot`(900 秒 + RunAtLoad)已加载,入口为 `deployment/macos/backup-snapshot-cycle.sh`(快照落本机 `backups/snap`,成功后仅镜像密文 objects→packages→latest.json 到 iCloud,`.staging`/`run.lock` 永不离机)。实测:93MB 生产库单轮 2.5s(VACUUM+加密 1674ms),写锁窗口可忽略;首恢复点 `2026-08-30T15:09:28Z`,`userVersion=10` 与部署清单 `reviewSchemaTarget` 吻合;**直接从 iCloud 镜像**恢复演练全绿(`quick_check/FK/user_version/sqlite_master/drill_public_pointer_verified=1`);连续三轮(手动 1 + launchd 2)轮转与内容哈希去重正确(`retainedCount=3`,同 contentHash 单对象 132MB)。踩坑:launchd 后台任务受 TCC 限制无权写 `~/Library/Mobile Documents`,首轮自动镜像 `Operation not permitted`;用户已向 `/bin/bash` 授予完全磁盘访问,复跑 `OFFHOST_MIRROR_OK`。回退方式:`launchctl bootout gui/<uid>/com.f1plus1.backup-snapshot` + 删除本机/iCloud 两个备份目录,生产库全程只读。**`BBFF2A` 的前置「verified 异机加密恢复点」自此满足**,下一步是其合法 control 链与 RSS 身份闭包。证据:`docs/decisions/evidence/2026-08-30-rpo-retier/snap-production-first-recovery-point.md`。
