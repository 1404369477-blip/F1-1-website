...
- 当前阶段:M5 本地 Build Loop；`public-synthetic` 与新增的 `public-multimedia-synthetic` SQLite、projection-first Repository/feed/detail API 均已核收。最终 v0.2 正式 App 落地任务 `TASK-20260808-33533E` 已 ACK；`TASK-20260809-BA9999` 的 `DEV-MM-01..03` 已完成独立 profile、原子 0/1/4 图 seed、默认 V1/精确 V2 API 与 fail-closed 后端，并经安全 `TASK-20260809-4A5381`、测试 `TASK-20260809-B98C66` 独立门禁 PASS 后由统筹 ACK。VS1 本地 synthetic operator 的 `COLLECT-MOCK-002`、`CONTENT-PROCESS-003`、`SUMMARY-MOCK-004` 已由开发 `D6114C/3A8C0E`、数据 `5A9316`、测试 `C66A73/9D61AD`、安全 `BCF8B1/D33AF3` 的 ACK 证据闭合为 `complete`；该完成只覆盖固定 Node24、本地 fixture、V-OP/25-case 恢复链与 `externalCalls=0`。`DEV-MM-04` 公开页 UI 接线、浏览器交互与视觉仍等待用户确认，不能外推为公开页完成。Admin 业务与视觉仍受独立门禁；未来生产拓扑已窄范围 accepted 为家中或办公室的专用常开 Admin MacBook + 独立公开只读主机，真实设备配置与生产部署继续未授权。真实 provider/Base/AI、采集与全部外部能力继续关闭或保持各自用户门禁
- 最近更新:2026-08-09

当前初版功能完整性以 [F1+1 初版全功能追踪矩阵 v0.1](spec/F1+1-初版全功能追踪矩阵-v0.1.md) 为唯一索引。对外状态只允许 `complete`、`user-gated`、`P1-blocker`；组件预留、静态分支、测试注入、占位或 `NOT_RUN` 不等于完成。矩阵当前计数为 `complete=17 / user-gated=20 / P1-blocker=13`。多图机器前置与独立运行链已闭合：`TASK-20260809-33B8F5`、`TASK-20260809-385B52`、`TASK-20260809-BA9999`、`TASK-20260809-4A5381`、`TASK-20260809-B98C66` 均已 ACK，`DEV-MM-01..03` 已覆盖完整 runtime graph、独立 SQLite/migration、原子 0/1/4 图 seed、Repository 与 V1/V2 API。VS1 三项 mock Function 已完成本地 operator、25-case 恢复链、数据事务/hash 与独立测试/安全门禁；Admin 可见性、真实 provider/Base/AI 和非 loopback 外部 I/O 不随之开放。`DEV-MM-04` 的公开 App V2 接线、浏览器交互和视觉仍受用户确认门禁约束；本地后端 PASS 不等于公开页面功能完成。
...
> [accepted] 当前系统路线入口包括 [ADR-SOURCE-001](decisions/system/2026-08-01-F1+1-信源库A到D演进路线-accepted.md)、[ADR-M4-KICKOFF-001](decisions/system/2026-08-01-F1+1-M4本地Kickoff系统路线-accepted.md)、[ADR-M4-PUBLIC-READ-001](decisions/system/2026-08-03-F1+1-公开读模型与API接线-v0.4-successor-accepted.md) 与窄范围 [ADR-M5-PUBLIC-MULTIMEDIA-RUNTIME-001](decisions/system/2026-08-09-F1+1-public-multimedia-synthetic运行profile与V2接线-successor-accepted.md)。公开前端早期 10 文件精确快照、v0.4 数据机器合同、双 profile SQLite/seed、受控启动出口及原 Repository/feed/detail API 已通过各自独立门禁；第三个 `public-multimedia-synthetic` profile 和 `DEV-MM-01..03` 后端也已通过开发、测试、安全闭包。`DEV-MM-04` 公开 UI 与 Admin successor、真实外部能力继续按独立门禁推进。

> [accepted] Admin 生产拓扑的窄范围入口为 [ADR-M5-ADMIN-DUAL-HOST-001](decisions/system/2026-08-09-F1+1-M5-Admin双主机拓扑-successor-accepted.md)与 [Admin 双主机实施合同 v0.2](spec/F1+1-M5-Admin双主机实施合同-v0.2.md)：独立 Admin 主机、唯一写主、Admin→public 单向只读投影、Mac/iPhone 全功能等价、`RTO≤4h`、`RPO≤15m`。该决定不批准 `ADR-M5-REVIEW-SYNTHETIC-001` 业务范围、Admin 视觉或生产部署；三者继续按独立门禁推进。
...
- 生产授权边界:本地 Kickoff 不授权 Base 业务真值切换、A `base_direct` 切换、D `base_snapshot` 切换、真实采集/表单提交、自动发布、部署、外发或付费。三道切换门禁仍分别需要实现验证、故障/回滚检查与用户确认。
- 中国大陆与个人维护: M4 不部署，运行时不依赖境外托管服务或远程密钥；未来部署必须另行确认运营主体、部署地域、数据区域、平台条款、AI 跨境与容量。网络安装受限时由用户配置合规 npm 镜像，仓库不保存凭证。
- 开工与实现证据:B 层 scaffold/lockfile 与 C 层 Node24.18.0/npm11.16.0/SQLite3.53.1 预检已通过；早期公开 feed/detail 前端 10 文件精确快照的开发、测试、安全任务均已 ACK。v0.4 数据机器合同整改复验、双 profile SQLite/seed、Next 受控启动出口、数据库覆盖单点整改，以及原 Repository/feed/detail API 的开发/测试/安全窗口也均已 ACK。多媒体 `DEV-MM-01..03` 的第三 profile、0/1/4 图原子 seed 与 V1/V2 API 已通过开发、测试、安全独立门禁；`DEV-MM-04`、完整 R12、Admin、生产存储和无障碍 AT 仍需各自收据。
...
- [ ] 本地页面路由固定为 `/`、`/stories/[publicId]`、`/admin/reviews`、`/admin/sources`。公开首页与详情只从 projection-first API/Repository 读取且无 `DEMO_STORIES`；最终 App 已落地，但最新候选必须先关闭矩阵中全部公开页 P1。审核队列和信源管理继续按矩阵门禁推进。
...
- [ ] 新信源仅在 canonical URL 有效、规范化有效、查重唯一、三门和五 fence 当前且 stop 清除时，通过一个事务原子写 `enabled=true`、`queued`、唯一 onboarding operation 与唯一 outbox；worker lease 后才 `queued → collecting`。platform > authorization > adapter 的阻断优先级、每个重试/恢复出口、`paused` 仅 lifecycle_status、`epoch=0` schema 拒绝和身份/相关性 unknown 不升级均有静态合同与 fixture/API/UI 路径。
